...
(function() {
  var Match, calculate_operations, consecutive_where, create_index, diff, find_match, find_matching_blocks, html_to_tokens, remove_style_tags, is_end_of_tag, is_start_of_tag, is_tag, is_whitespace, isnt_tag, op_map, recursively_find_matching_blocks, render_operations, wrap;
  ...
  remove_style_tags = function(tokens, tags) {
    return tokens.map(function(token) {
      var remove = false;

      for (var i = 0; i < tags.length; i++) {
        if (is_tag_type(token, tags[i])) {
          remove = true;
        }
      }

      if (!remove || !token.match(/style=".*"/)) {
        return token;
      }
  ...
  diff = function(before, after) {
    ...
    // compare without style changes for tags below (it otherwise adds extra table cells)
    var ignore = ['td'];
    ops = calculate_operations(
      remove_style_tags(before, ignore),
      remove_style_tags(after, ignore)
    );
